...
class MainMenu extends React.Component { // eslint-disable-line
  ...
  filterMenuRole = (item, role) => {    
    return !!(item && item.roles && item.roles.length && item.roles.includes(role));
  }
  ...
  render() {
    ...
    .filter(item => this.filterMenuRole(item, profileData && profileData.role))
    .map((item, index) => {
      if (item.child) {
        return (
